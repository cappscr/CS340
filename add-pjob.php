...
<?php
	...
	if (!($stmt = $mysqli->prepare("INSERT INTO people_jobs (person_id,game_id,job_id,develop_id) VALUES (?,?,?,?)"))){
		echo "Prepare failed: " . $stmt->errno . " " . $stmt->error;
	}

	if(!($stmt->bind_param("iiii", $_POST['pid'], $_POST['vgid'], $_POST['jid'], $_POST['did']))){
	...
	if(!$stmt->execute()){
		...
		echo "Inserted " . $stmt->affected_rows . " rows into people_jobs.";
	}
