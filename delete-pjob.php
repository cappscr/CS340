...
<?php
	...
	if (!($stmt = $mysqli->prepare("DELETE FROM people_jobs WHERE person_id = ? AND job_id = ? AND game_id = ?"))){
		echo "Prepare failed: " . $stmt->errno . " " . $stmt->error;
	}

	if(!($stmt->bind_param("iii", $_POST['pid'], $_POST['jid'], $_POST['gid']))){
	...
	if(!$stmt->execute()){
		...
		echo "Deleted " . $stmt->affected_rows . " rows from people_jobs.";
	}
