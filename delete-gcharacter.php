<!-- **************************************
**** filename: delete-gcharacter.php
...
<?php
	// Prepare a SQL statement to delete the values for a selected row in the game_char table 
	// the values to select the correct row are passed via a form to this page
	if (!($stmt = $mysqli->prepare("DELETE FROM game_char WHERE char_id = ? AND game_id = ?"))){
		echo "Prepare failed: " . $stmt->errno . " " . $stmt->error;
	}

	if(!($stmt->bind_param("ii", $_POST['cid'], $_POST['gid']))){
	...
	if(!$stmt->execute()){
		...
		echo "Deleted " . $stmt->affected_rows . " rows from game_char.";
	}
